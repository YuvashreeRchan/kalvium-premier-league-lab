...
{
  let position_players=[];
  players.forEach(function(player){
    if(player.position==position){
      position_players.push(player);
    }
  });
...
function filterByAwardxCountry(awardName,country){
  let players_country = players.filter((player) => 
  {
    return player.country === country;
  });

  let selected = players_country.filter((player) => 
  {
...
function filterByNoOfAwardsxTeamxAge(noOfAwards, team, age){
  return players ? players.filter(player => 
    player.team == team && player.age < age && player.awards.length >= noOfAwards): [];
}
...
function SortByNamexAwardxTimes(awardName, noOfTimes, country){
  return awardName ? players.filter((player) =>
